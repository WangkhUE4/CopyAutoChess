...
class BPEnumMember:
    def __init__(self, name, value, common):
        self.name = name
        self.common = common
        self.value = value

    def out(self,export_bp):
        t = ""
        t += "\t" + self.name

        if export_bp:
            t += ' = ' + self.value + ' UMETA(DisplayName="' + self.name + '"),'
        else:
            t += ' = ' + self.value + ','
...
class BPEnum:
    def __init__(self):
        ...
        self.common = ""
        self.members = []
        self.is_export_bp = True

    def setup(self):
        """
        fixed not export_bp when member count >= 255 and member value >= 255.

        :return:
        """
        self.is_export_bp = len(self.members) < 255
        for m in self.members:
            if int(m.value) >= 255 or int(m.value) < 0:
                self.is_export_bp = False
                break
    ...
    def out(self):
        ...
        if self.is_export_bp:
            ...
            t += "enum class " + self.name + ": int32 {\n"

        for m in self.members:
                t += m.out(self.is_export_bp)
...
class BPDef:
    ...
    def add_child_ref_msg_count(self):
        pass

    def add_ref_msg_count_final(self):
        pass
...
class BPMsgDef(BPDef):
    ...
    def add_child_ref_msg_count(self):
        child = self.encode.find_msg(self.type)
        if child is not None:
            if child.ref_count <= self.owner_msg.ref_count:
                child.ref_count = self.owner_msg.ref_count + 1
                self.add_ref_msg_count_final()

    def add_ref_msg_count_final(self):
        tm = self.encode.find_msg(self.type)
        if tm is not None:
           for child in tm.members:
               child.add_child_ref_msg_count()
...
class BPArrayDef(BPDef):
    ...
    def add_child_ref_msg_count(self):
        self.array_type.add_child_ref_msg_count()


    def add_ref_msg_count_final(self):
        self.array_type.add_ref_msg_count_final()
...
class BPMapDef(BPDef):
    ...
    def add_child_ref_msg_count(self):
        self.key_type.add_child_ref_msg_count()
        self.value_type.add_child_ref_msg_count()


    def add_ref_msg_count_final(self):
        self.key_type.add_ref_msg_count_final()
        self.value_type.add_ref_msg_count_final()
...
class BPEncode:
    ...
    def create_enum_member(self, n, v, c):
        n = fuck_pb_name(n)
        return BPEnumMember(n, v, c)
    ...
    def create_msg_def(self, n, t, c, om):
        ...
        if is_export_bp:
            if t.startswith('TArray<'):
                et = 'E' + t.replace('TArray', '').replace('<', '').replace('>', '')
                e = self.find_enum(et)
                if e:
                    is_export_bp = e.is_export_bp
            else:
                if t.startswith('TMap<'):
                    m = t.replace('TMap', '').replace('<', '').replace('>', '')
                    keywords = m.split(',')
    ...
    def reorder_message_final(self):
        for m in self.message_list:
            for mb in m.members:
                mb.add_ref_msg_count_final()
            ...
            for mm in m.members:
                if mm.is_export_bp:
                    ...
                    if t.startswith('TArray<'):
                        ...
                        e = self.find_enum(et)
                        if e:
                            mm.is_export_bp = e.is_export_bp
                    else:
                        if t.startswith('TMap<'):
                            m = t.replace('TMap', '').replace('<', '').replace('>', '')
                            keywords = m.split(',')
            ...
            if c.type == PCT_BeginEnum:
                ...
                if self.current_block:
                    if isinstance(self.current_block, BPEnum):
                        self.current_block.members.append(self.create_enum_member(c.tokens[0], c.tokens[1], c.common))
    ...
    def parse(self, code_list=[]):
        ...
        while True:
            ...
            if c.type == PCT_BeginMessage:
                ...
                if not isinstance(self.current_block, BPEnum):
                    if c.tokens[0] == 'repeated':
                        msg_def = self.create_msg_def(c.tokens[2], 'TArray<' + c.tokens[1] + '>', c.common,
                                                      self.current_block)
                        ...
                        msg_def = self.create_msg_def(c.tokens[2], 'TMap<' + first + ', ' + second + '>', c.common,
                                                      self.current_block)
                        self.current_block.members.append(msg_def)
                    else:
                        msg_def = self.create_msg_def(c.tokens[1], c.tokens[0], c.common, self.current_block)
                        self.current_block.members.append(msg_def)
...
#pyinstaller  -F ProtoToBp.py ProtoParser.py